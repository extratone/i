...
static JSValuePtr jsDOMWindowBaseOrientation(ExecState*, const Identifier&, const PropertySlot&);
...
static JSValuePtr jsDOMWindowBaseTouch(ExecState*, const Identifier&, const PropertySlot&);
static void setJSDOMWindowBaseTouch(ExecState*, JSObject*, JSValuePtr);
static JSValuePtr jsDOMWindowBaseTouchList(ExecState*, const Identifier&, const PropertySlot&);
static void setJSDOMWindowBaseTouchList(ExecState*, JSObject*, JSValuePtr);
...
# -- Attributes --
  ...
  orientation                   jsDOMWindowBaseOrientation                  DontDelete|ReadOnly
# -- Constructors --
...
# ENABLE(TOUCH_EVENTS)
  Touch                         jsDOMWindowBaseTouch                        DontDelete
  TouchList                     jsDOMWindowBaseTouchList                    DontDelete
@end
*/
...
JSValuePtr jsDOMWindowBaseOrientation(ExecState*, const Identifier&, const PropertySlot&)
{
    // FIXME: DDK: Missing code?
    return jsUndefined(); // FIXME: implement this
}
...
JSValuePtr jsDOMWindowBaseTouch(ExecState* exec, const Identifier&, const PropertySlot& slot)
{
    if (!static_cast<JSDOMWindowBase*>(asObject(slot.slotBase()))->allowsAccessFrom(exec))
        return jsUndefined();
    return getDOMConstructor<JSTouchConstructor>(exec, static_cast<JSDOMWindowBase*>(asObject(slot.slotBase())));
}

JSValuePtr jsDOMWindowBaseTouchList(ExecState* exec, const Identifier&, const PropertySlot& slot)
{
    if (!static_cast<JSDOMWindowBase*>(asObject(slot.slotBase()))->allowsAccessFrom(exec))
        return jsUndefined();
    return getDOMConstructor<JSTouchListConstructor>(exec, static_cast<JSDOMWindowBase*>(asObject(slot.slotBase())));
}
...
void setJSDOMWindowBaseTouch(ExecState* exec, JSObject* thisObject, JSValuePtr value)
{
    if (!static_cast<JSDOMWindowBase*>(thisObject)->allowsAccessFrom(exec))
        return;
    // Shadowing a built-in constructor
    static_cast<JSDOMWindowBase*>(thisObject)->putDirect(Identifier(exec, "Touch"), value);
}

void setJSDOMWindowBaseTouchList(ExecState* exec, JSObject* thisObject, JSValuePtr value)
{
    if (!static_cast<JSDOMWindowBase*>(thisObject)->allowsAccessFrom(exec))
        return;
    // Shadowing a built-in constructor
    static_cast<JSDOMWindowBase*>(thisObject)->putDirect(Identifier(exec, "TouchList"), value);
}
...
{
    int timeoutId = DOMTimer::install(scriptExecutionContext(), a, t, singleShot);

    bool deferTimeout = (impl()->frame() && impl()->frame() && impl()->frame()->timersPaused());
    if (!deferTimeout) {
        DOMTimer *timer = impl()->frame()->document() ? impl()->frame()->document()->findTimeout(timeoutId) : 0;
        if (t <= 100 && singleShot && timer) {
