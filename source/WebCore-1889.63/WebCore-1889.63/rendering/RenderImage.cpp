...
void RenderImage::collectSelectionRects(Vector<SelectionRect>& rects, unsigned, unsigned)
{
    ...
    if (!inlineBox) {
        ...
        LayoutUnit selectionTop = !cb->style()->isFlippedBlocksWritingMode() ? inlineBox->root()->selectionTop() - logicalTop()
        ...
        LayoutUnit leftOffset = cb->logicalLeftSelectionOffset(cb, inlineBox->logicalTop(), cache);
        LayoutUnit rightOffset = cb->logicalRightSelectionOffset(cb, inlineBox->logicalTop(), cache);
    ...
    rects.append(SelectionRect(absoluteBounds, cb->style()->direction(), lineExtentBounds.x(), lineExtentBounds.maxX(), lineExtentBounds.maxY(), 0, false, isFirstOnLine, isLastOnLine, false, false, cb->style()->isHorizontalWritingMode(), isFixed, false, columnNumberForOffset(absoluteBounds.x())));
}
#endif
