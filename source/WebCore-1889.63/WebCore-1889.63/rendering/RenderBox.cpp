...
RoundedRect::Radii RenderBox::borderRadii() const
{
    ...
    int borderLeft = style->borderLeftWidth();
    int borderTop = style->borderTopWidth();
    ...
    RoundedRect border = style->getRoundedBorderFor(bounds);
    return border.radii();
}
#endif
...
LayoutRect RenderBox::layoutOverflowRectForPropagation(RenderStyle* parentStyle) const
{
    ...
    bool hasTransform = hasLayer() && layer()->transform();
#if PLATFORM(IOS)
    if (hasPaintOffset() || (hasTransform && (!document()->settings() || document()->settings()->shouldTransformsAffectOverflow()))) {
#else
