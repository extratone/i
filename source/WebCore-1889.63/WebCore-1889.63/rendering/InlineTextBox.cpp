...
#if PLATFORM(IOS)
        if (styleToUse->textDecorationsInEffect() & LETTERPRESS)
            context->setTextDrawingMode(context->textDrawingMode() | TextModeLetterpress);
        else
            context->setTextDrawingMode(context->textDrawingMode() & ~TextModeLetterpress);
#endif
...
#else
    // On iOS we want to draw crisp decorations. drawLineForText takes the context's strokeThickness
    // and renders that at device pixel scale. i.e. a strokeThickness of 1 will produce
    // a 1 device pixel line, so thinner on retina than non-retina.
    // We will also scale our thickness based on the size of the font. Since our
    // default size is 16px we'll use that as a scale reference.
    float documentScale = renderer()->frame()->documentScale();
    const float textDecorationBaseFontSize = 16.0f;
    float fontSizeScaling = renderer()->style()->fontSize() / textDecorationBaseFontSize;
    float strokeThickness = roundf(textDecorationThickness * fontSizeScaling * documentScale);
    context->setStrokeThickness(strokeThickness);
#endif
