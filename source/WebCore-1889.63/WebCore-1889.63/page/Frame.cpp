...
#if PLATFORM(IOS)
    // Ensure that document scheduled task state matches frame timer state.
    // It can be out of sync if timers state changed while the document was not in the frame (possibly in page cache, or simply newly created).
    // FIXME: How does this interact with cross-platform code below?
    if (m_doc) {
        if (timersPaused())
            m_doc->suspendScheduledTasks(ActiveDOMObject::DocumentWillBePaused);
        else
            m_doc->resumeScheduledTasks(ActiveDOMObject::DocumentWillBePaused);
    }
#endif
